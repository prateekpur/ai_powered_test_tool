...
def generate_security_tests(
    ...
    template = _load_prompt_template("generate_security_tests_prompt.txt")
    sections = [
        f"{template}\n",
        f"## Requirement\n\n{requirement.to_markdown()}",
        f"## Analysis\n\n{analysis.to_markdown()}",
...
def validate_test_schema(
    ...
    if security_tests.strip():
        security_template = _load_prompt_template("validate_security_tests_prompt.txt")
        sections.append(f"{security_template}\n\n## Security Tests\n\n{security_tests.strip()}")
    return "\n\n".join(sections)
